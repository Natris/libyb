...
public:
	...
	task<short> cancel_and_wait() throw() override
	{
		...
		{
			return task<short>::from_exception(std::make_exception_ptr(task_cancelled()));
		}
	}
	...
	{
		return detail::dbg_print(ctx + 1, "linux_fdpoll_task: m_fd=%d, m_events=%d", (int)m_fd, (int)m_events);
	}
